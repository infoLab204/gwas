...
stest <- function(geno1, geno2, pheno, pca_num) {
    ...
    if(is.null(pca_num)) stop("Error: enter number of principal components")
...
#        pearson_skewness[i] <- (sqrt(n*(n-1))/(n-2))*(pearson_m3/((pearson_m2)^(3/2)))
    ...
    par(mfrow=c(1,2))	 
    boxplot(skew,col=c("orange","white"), border=c("black","black"), main="Skewness", names=c("Kendall","Pearson"),ylim=c(-1.5,4))
    boxplot(kurt,col=c("orange","white"), border=c("black","black"), main="Kurtosis", names=c("Kendall","Pearson"), ylim=c(-1.5,4))
    dev.off()
